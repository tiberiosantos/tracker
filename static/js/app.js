...
const projectData = (filters, options) => {
...
const renderData = (result) => {
  return new Promise((resolve, reject) => {
    ...
    result.forEach((bulk) => {
      renderMarker(bulk, marker_icon, marker_color, tables).then((pointer) => {
...
const renderPopup = (bulk) => {
  ...
  data.popup.content.forEach((item) => {
    if (item.hasOwnProperty("format")) {
      bulk[item.data_field] = format(bulk[item.data_field], item.format);
    }
    template += `<span>
...
const renderMarker = (bulk, marker_icon, marker_color, tables) => {
...
const updateResult = (tables, item, value) => {
  if (tables) {
